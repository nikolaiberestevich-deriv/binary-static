...
const List = ({ arr, saveRef, underlying, onUnderlyingClick }) =>
  arr.map(([market_code, obj], idx) => (
    ...
    >
      <div className="market_name">{obj.name}</div>
      {Object.entries(obj.submarkets)
        .sort((a, b) => sortSubmarket(a[0], b[0]))
        .map(
          (
            [key, submarket],
            idx_2 // eslint-disable-line no-unused-vars
          ) => (
            <div className="submarket" key={idx_2}>
              <div className="submarket_name">{submarket.name}</div>
              <div className="symbols">
                {Object.entries(submarket.symbols).map(([u_code, symbol]) => (
                  <div
                    className={`symbol_name ${
                      u_code === underlying ? "active" : ""
                    }`}
                    key={u_code}
                    id={u_code}
...
class Markets extends React.Component {
  constructor(props) {
    ...
    let final_markets_arr, final_market_obj;
    ...
    if (!underlying_symbol || !this.underlyings[underlying_symbol]) {
      const submarket = Object.keys(
        this.markets[market_symbol].submarkets
      ).sort(sortSubmarket)[0];
      underlying_symbol = Object.keys(
        this.markets[market_symbol].submarkets[submarket].symbols
      ).sort()[0];
    }
    ...
    const is_malta =
      State.getResponse("landing_company.gaming_company.shortcode") === "malta";
    const market_arr = Object.entries(this.markets).sort((a, b) =>
      sortSubmarket(a[0], b[0])
    );
    if ((is_malta || is_uk) && Client.getAccountOfType("virtual")) {
      final_markets_arr = market_arr.filter((market) => is_synthetic(market));
      final_market_obj = final_markets_arr.reduce(
        (acc, it) => ({ ...acc, [it[0]]: it[1] }),
        {}
      ); // works as Object.fromEntries(final_markets_arr) but some browsers don't support Object.fromEntries => trackjs errors
      market_symbol = Object.keys(final_market_obj)[0];
      const submarket = Object.keys(
        final_market_obj[market_symbol].submarkets
      ).sort(sortSubmarket)[0];
      underlying_symbol = Object.keys(
        final_market_obj[market_symbol].submarkets[submarket].symbols
      ).sort()[0];
    } else {
    ...
    if (!(market_symbol in this.markets)) {
      market_symbol = Object.keys(this.markets).find(
        (m) => this.markets[m].submarkets[market_symbol]
      );
  ...
  openScrollMonitor = (element) => {
    ...
    this.setState({
      open_dropdown_scroll_id: setInterval(
        forceScroll.bind(null, element),
        300
      ),
    });
    setTimeout(() => {
  ...
  onUnderlyingClick = (underlying_symbol, market_symbol) => {
    ...
    this.el_underlying.value = underlying_symbol;
    this.el_underlying.setAttribute(
      "data-text",
      this.underlyings[underlying_symbol]
    );
  ...
  stickyHeader = (position) => {
    ...
    const current_viewed_node = Object.values(market_nodes).find(
      (node) =>
    ...
    const diff =
      +current_viewed_node.dataset.offsetHeight +
      +current_viewed_node.dataset.offsetTop -
      position;
    if (diff > 0 && diff < TITLE_HEIGHT) {
      current_viewed_node.children[0].style.top = `${
        DEFAULT_TOP - (TITLE_HEIGHT - diff)
      }px`;
  ...
  searchSymbols = ({ target: { value: query } }) => {
    ...
    this.setState({
      ...
      active_market: filter_markets[0][0],
    });
  };
  ...
  render() {
    const { active_market, markets, underlying, query, market, open } =
      this.state;
    const {
      getCurrentUnderlying,
      openDropdown,
    ...
    return (
      <div className="markets">
        <div className="market_current" onClick={openDropdown}>
...
export const init = () => {
  ReactDOM.render(<Markets />, getElementById("underlying_component"));
};
